...
namespace GameBox
{
    public static partial class BetterStreamingAssets
    {
        ...
        public static void Initialize()
        {
            if (Application.isEditor)
            {
                EditorImpl.Initialize(Application.dataPath, Application.streamingAssetsPath);
            }
            else if (Application.platform == RuntimePlatform.Android)
            {
                ApkImpl.Initialize(Application.dataPath, Application.streamingAssetsPath);
            }
            else
            {
                LooseFilesImpl.Initialize(Application.dataPath, Application.streamingAssetsPath);
            }
        }
        ...
        {
            EditorImpl.ApkMode = true;
            EditorImpl.Initialize(apkPath, "jar:file://" + apkPath + "!/assets/");
        }

        public static void InitializeWithExternalDirectories(string dataPath, string streamingAssetsPath)
        {
            EditorImpl.ApkMode = false;
            EditorImpl.Initialize(dataPath, streamingAssetsPath);
        }
        ...
        public static bool FileExists(string path)
        {
            ...
            if (Application.isEditor)
            {
                return EditorImpl.TryGetInfo(path, out info);
            }
            else if (Application.platform == RuntimePlatform.Android)
            {
        ...
        public static bool DirectoryExists(string path)
        {
            if (Application.isEditor)
            {
                return EditorImpl.DirectoryExists(path);
            }
            else if (Application.platform == RuntimePlatform.Android)
            {
        ...
        public static AssetBundleCreateRequest LoadAssetBundleAsync(string path, uint crc = 0)
        {
            ...
            if (GetInfo(path, out ReadInfo info))
            {
        ...
        public static System.IO.Stream OpenRead(string path)
        {
            ...
            if (Application.isEditor)
            {
                return EditorImpl.OpenRead(path);
            }
            else if (Application.platform == RuntimePlatform.Android)
            {
        ...
        public static byte[] ReadAllBytes(string path)
        {
            ...
            if (Application.isEditor)
            {
                return EditorImpl.ReadAllBytes(path);
            }
            else if (Application.platform == RuntimePlatform.Android)
            {
        ...
        public static string[] GetFiles(string path, string searchPattern, SearchOption searchOption)
        {
            if (Application.isEditor)
            {
                return EditorImpl.GetFiles(path, searchPattern, searchOption);
            }
            else if (Application.platform == RuntimePlatform.Android)
            {
        ...
        #region GURU_DEV
        private static bool GetInfo(string path, out ReadInfo result)
        {
            bool is_success;

            if (Application.isEditor)
            {
                is_success = EditorImpl.TryGetInfo(path, out result);
            }
            else if (Application.platform == RuntimePlatform.Android)
            {
                is_success = ApkImpl.TryGetInfo(path, out result);
            }
            else
            {
                is_success = LooseFilesImpl.TryGetInfo(path, out result);
            }

            if (!is_success)
            {
                Debug.LogError($"[BSA]: File ({path}) not found!");
            }
        ...
        internal static class ApkImpl
        {
            ...
            public static void Initialize(string dataPath, string streamingAssetsPath)
            {
                ...
                // maybe in split apk?
                var split_DataPath = Path.GetDirectoryName(dataPath) + "/split_base_assets.apk";

                if (File.Exists(split_DataPath))
                {
                    s_root = split_DataPath;
                    GetStreamingAssetsInfoFromJar(split_DataPath, paths, parts);
                }
                else
                {
                ...
                Debug.Log($"[BSA]Initialize: Final path s_root = {s_root}");
